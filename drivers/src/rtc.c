 ...
 * Define
 */
#define RTC_LSE_TIMEOUT				1000000
...
bool rtc_init(void){
	...
	if((RCC->BDCR & RCC_BDCR_RTCEN) == 0){					// RTC clock is enable
		...
		PWR->CR1		|= PWR_CR1_DBP;						// Allow access to Backup
...
bool rtc_setTime(const struct tm *t){
	...
	if((t->tm_sec > 59) || (t->tm_min > 59) || (t->tm_hour > 23) || (t->tm_mday > 31)
			|| (t->tm_mon > 12) || (t->tm_year < 100) || (t->tm_year > 200)){
		return false;
	}
