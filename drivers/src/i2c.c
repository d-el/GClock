 ...
 * @version 	V1.2
 * @date		22.11.2024
 * @copyright	The MIT License (MIT). Copyright (c) 204 Storozhenko Roman
 */
/*!****************************************************************************
* Include
*/
...
void i2c_init(i2c_type *i2cx){
	...
	if(i2cx == i2c2){
		...
		*/
		i2cx->pI2c			= I2C2;
		i2cx->pDmaTxDmaCh	= DMA1_Channel2;
		i2cx->DMA_MUX		= DMAMUX1_Channel1;
		...
		* PB14 - I2C2_SDA
		*/
		gppin_init(GPIOA, 11, alternateFunctionOpenDrain, pullUp, 0, I2C2_PINAFSCL);
		gppin_init(GPIOA, 12, alternateFunctionOpenDrain, pullUp, 0, I2C2_PINAFSDA);

		/************************************************
		* NVIC
		*/
		NVIC_EnableIRQ(I2C2_IRQn);											// Event interrupt enable
		NVIC_SetPriority(I2C2_IRQn, I2C2_EventInterruptPrior);

		/************************************************
		* I2C clock
		*/
		RCC->APBENR1	|= RCC_APBENR1_I2C2EN;								// USART1 clock enable
		RCC->APBRSTR1	|= RCC_APBRSTR1_I2C2RST;							// USART1 reset
	...
	* I2C
	*/
	i2cx->pI2c->CR1	|= I2C_CR1_TXDMAEN;								// DMA mode enabled for transmission
	...
	i2cx->pI2c->TIMINGR |= (0x2) << I2C_TIMINGR_PRESC_Pos;
...
void i2c_write(i2c_type *i2cx, const void *src, uint16_t len, uint8_t slaveAdr, i2c_stopMode_type stopMode){
	...
	i2cx->DMA_MUX->CCR &= ~(0x7F << DMAMUX_CxCR_DMAREQ_ID_Pos);
	i2cx->DMA_MUX->CCR = 13 << DMAMUX_CxCR_DMAREQ_ID_Pos;	// I2C2_TX
...
void i2c_read(i2c_type *i2cx, void *dst, uint16_t len, uint8_t slaveAdr){
	i2cx->pDmaTxDmaCh->CCR &= ~DMA_CCR_EN;
	i2cx->pDmaTxDmaCh->CCR &= ~DMA_CCR_DIR;					// Read from peripheral
	i2cx->pDmaTxDmaCh->CMAR = (uint32_t)dst;
	i2cx->pDmaTxDmaCh->CPAR = (uint32_t)&i2cx->pI2c->RXDR;	// Peripheral address
	i2cx->pDmaTxDmaCh->CNDTR = len;
	i2cx->DMA_MUX->CCR &= ~(0x7F << DMAMUX_CxCR_DMAREQ_ID_Pos);
	i2cx->DMA_MUX->CCR = 12 << DMAMUX_CxCR_DMAREQ_ID_Pos;	// I2C2_RX
	i2cx->slaveAdr = slaveAdr;
	...
	i2cx->pI2c->CR2	&= ~I2C_CR2_SADD;						// Slave address bit (master mode)
	i2cx->pI2c->CR2	|= (slaveAdr) << I2C_CR2_SADD_Pos;		// Slave address bit (master mode)
